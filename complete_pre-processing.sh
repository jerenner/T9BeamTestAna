...
#     #this code does the peak finding and timing and creates the first ntuple, careful at this stage the signalTimeCorrected branch doesn't actually have the corrected timings
    python python/new_analysis/process_waveform_analysis.py data/root_run_$run.root config/config_hodoscope.json peakAnalysed_$run.root
#
...
#     root -l -b -q 'macros/timeCorrection.C("peakAnalysed_'$run'.root", "triggerTimeDrift.txt", "peakAnalysed_timeCorr_'$run'.root")'
...
#     python python/new_analysis/process_ac_waveform_analysis.py data/root_run_$run.root peakAnalysed_timeCorr_$run.root -16 45 config/config_ac_noHodoscope.json peakAnalysed_timeCorr_windIntCorrMatched_-16_45_$run.root
