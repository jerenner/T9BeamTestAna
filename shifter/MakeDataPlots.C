...
void MakeDataPlots(string fileName, int momentum, TString peakMode = "") {
  ...
  TH1I hnHitsHodoscope("hnHitsHodoscope", ";Hodoscope channel; Number of hits", 15, 0.5, 15.5);
  ...
  for(int i = 0; i < ent; i++) {
    ...
    // jiri
    double pbanew = peakVoltage[16][0];
    int maxhdchid = -1;
    double maxA = -1;
    bool hits[15];
    for(int j = 0; j < 15; ++j)
  ...
  for(int j = 0; j < 15; ++j) {
    double diag = HodoOccScatter.GetBinContent(j,j);
    cout << "diag=" << diag << endl;
    for(int k = 0; k <= j; ++k) {
	if (diag > 0.)
	  HodoOccScatterFrac.SetBinContent(j, k, HodoOccScatter.GetBinContent(j,k) / diag);
    } // k
  } // j
  HodoOccScatterFrac.Scale(1.);
  HodoOccScatter.Scale(1.);
